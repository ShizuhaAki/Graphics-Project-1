...
void VertexRecorder::draw(GLenum mode) {
    ...
    glBufferData(GL_ARRAY_BUFFER, position_nbytes, m_position.data(),
                 GL_DYNAMIC_DRAW);

    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(m_position[0]),
                          (void *)0);

    // NORMALS
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer[1]);
    size_t normal_nbytes = m_nverts * sizeof(m_normal[0]);
    glBufferData(GL_ARRAY_BUFFER, normal_nbytes, m_normal.data(),
                 GL_DYNAMIC_DRAW);
    glEnableVertexAttribArray(1);
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(m_normal[0]),
                          (void *)0);

    // COLOR
    glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer[2]);
    size_t color_nbytes = m_nverts * sizeof(m_color[0]);
    glBufferData(GL_ARRAY_BUFFER, color_nbytes, m_color.data(),
                 GL_DYNAMIC_DRAW);
    glEnableVertexAttribArray(2);
    glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, sizeof(m_color[0]),
                          (void *)0);
